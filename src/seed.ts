...
const seed = async () => {
  ...
  for (const category of categories) {
    ...
    try {
      ...
      console.log(`✓ Created parent: ${parentCategory.slug}`);

      await new Promise((resolve) => setTimeout(resolve, 100));
    } catch (error) {
      console.error(`Error creating parent ${category.slug}:`, error);
      throw error;
    }
  }
  ...
  for (const parent of createdParents) {
    for (const subCategory of parent.subcategories) {
      ...
      try {
        ...
        console.log(`✓ Created subcategory: ${subCat.slug}`);

        await new Promise((resolve) => setTimeout(resolve, 100));
      } catch (error) {
        console.error(`Error creating subcategory ${subCategory.slug}:`, error);
        throw error;
      }
    }
  }
